...
namespace LiquorShopService.Implementations
{
    public class PurchaseService : DbService, IPurchaseService
    {
        ...
        {
            return await Context.Purchases.Where(a => (a.PurchaseDate) >= Start.Date && (a.PurchaseDate) <= End.Date)
                .ToListAsync();
        }
        ...
        public async Task<IEnumerable<Purchase>> FindPurchasesByBeverageAndDateAsync(DateTime Start, DateTime End, Beverage Beverage = null)
        {
            if (Beverage == null)
                return await Context.Purchases
              .Where(a => (a.PurchaseDate) >= Start.Date
               && (a.PurchaseDate) <= End.Date)
              .ToListAsync();
            return await Context.Purchases
        ...
        {
            return await Context.Purchases.Where(a =>(a.PurchaseDate)>=start &&
            (a.PurchaseDate) <= end)
